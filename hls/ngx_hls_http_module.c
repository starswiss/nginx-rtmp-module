...
static char * ngx_hls_http(ngx_conf_t *cf, ngx_command_t *cmd, void *conf);
...
ngx_hls_http_m3u8_handler(ngx_http_request_t *r, ngx_rtmp_addr_conf_t *addr_conf)
{
    ...
    if (s == NULL) {
        ...
        if (s == NULL) {
            ngx_log_error(NGX_LOG_ERR, r->connection->log, 0,
                "hls-http: m3u8_handler| create hls session error", &ctx->sid);
...
static char *
ngx_hls_http(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
