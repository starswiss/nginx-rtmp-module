...
char *
ngx_hls_set_msec_slot(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ngx_mpegts_gop_app_conf_t   *gacf;

    gacf = conf;
    gacf->cache_time = 10000;
        ...
        ngx_mpegts_frame_t *frame)
{
    ...
    if (ctx->keyframe == NULL) {
        if (frame->pts - ctx->cache[ctx->gop_pos]->pts
                > cache_time)
        {
...
ngx_mpegts_gop_cache(ngx_rtmp_session_t *s, ngx_mpegts_frame_t *frame)
{
    ...
    ngx_log_error(NGX_LOG_DEBUG, s->log, 0,
            ...
            frame->type, frame->keyframe, frame->pts, frame->length);
#endif
...
ngx_mpegts_gop_send_gop(ngx_rtmp_session_t *s, ngx_rtmp_session_t *ss)
{
    ...
    while (gacf->roll_back && keyframe &&
        ((sctx->current_timestamp - keyframe->pts) > ss->roll_back))
    {
        ngx_log_error(NGX_LOG_DEBUG, s->log, 0,
    ...
    while (frame) {
        ...
        if (!gacf->send_all &&
            frame->pts - ssctx->first_timestamp >= gacf->one_off_send)
        {
    ...
    ngx_msec_t time_offset, ngx_msec_t duration)
{
    ...
    while (frame) {
        ...
        if (frame->type == ssctx->base_type &&
            frame->pts - ssctx->first_timestamp >= duration)
        {
